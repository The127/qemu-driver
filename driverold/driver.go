...
import (
	...
	"github.com/gwenya/qemu-driver/driverold/machineconfig"
...
const (
	...
	ConfigJson          StorageFilename = "config.json"
...
type Driver interface {
	Create(imageSourcePath string) error
	Start() error
	Stop() error
	Reboot() error
	GetState() Status
...
type driver struct {
	...
	config    machineconfig.MachineConfiguration
	mu        sync.Mutex
	qemuPidFd int
	mon       qmp.Monitor
}
...
func New(opts Options) (Driver, error) {
	...
	config, err := configstorage.ReadFromFile(d.storagePath(ConfigJson))
	switch {
	case errors.Is(err, os.ErrNotExist): //do nothing
	case err != nil:
		return nil, fmt.Errorf("reading config: %w", err)
	default:
		d.config, err = machineconfig.MapConfigFromStorage(config)
		if err != nil {
	...
	if !strings.Contains(cmdline, d.id.String()) {
		err := os.Remove(pidFilePath)
		if err != nil {
			return nil, fmt.Errorf("failed to remove old pid file %q: %w", pidFilePath, err)
		}
	...
	err = d.queryConfigFromQmp()
	if err != nil {
		return nil, fmt.Errorf("querying running config from monitor: %w", err)
	}
...
func (d *driver) startWatcher() error {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				d.l.Logf("epoll_wait failed: %v", err)
		...
		if d.mon != nil {
			...
			if err != nil {
				d.l.Logf("closing qmp socket failed: %v\n", err)
			}
			d.mon = nil
		}
		d.signalExit()
...
func (d *driver) Create(imageSourcePath string) error {
	if d.qemuPidFd != -1 {
...
func (d *driver) connectMonitor() (qmp.Monitor, error) {
	if d.mon != nil {
		...
		d.l.Log("connection to qmp socket is broken, reconnecting")
		_ = d.mon.Disconnect()
	}
	...
	for {
		...
		if errors.Is(err, unix.ENOENT) || errors.Is(err, unix.ECONNREFUSED) {
			d.l.Log("qmp socket is not ready yet, retrying in a second")
...
func (d *driver) Stop() error {
	...
	err = d.mon.Disconnect()
	if err != nil {
		d.l.Logf("failed to close qmp: %v", err)
	}
...
func (d *driver) GetState() Status {
	...
	if d.qemuPidFd == -1 {
		...
		if err != nil {
			d.l.Logf("failed to check creation flag existence: %v\n", err)
			return Unknown
		}
	...
	mon, err := d.connectMonitor()
	if err != nil {
		d.l.Logf("failed to connect qmp monitor: %v\n", err)
		return Unknown
	}

	qemuStatus, err := mon.Status()
	if err != nil {
		d.l.Logf("failed to query qemu status: %v\n", err)
		return Unknown
	}
...
func (d *driver) queryConfigFromQmp() error {
	mon, err := d.connectMonitor()
	if err != nil {
		return err
	}
	...
	if err != nil {
		return fmt.Errorf("querying CPUs: %w", err)
	}

	d.config.CpuCount = uint32(len(cpus))
	...
	d.config.MemorySize = memory.Base + memory.Hotplugged
...
func (d *driver) SetCPUs(desiredCpus int) error {
	d.config.CpuCount = desiredCpus
	storeConfigToJson()

	currentCpus := getCurrentCpuCountFromQemu()

	if currentCpus == desiredCpus {
		return nil
	}

	if desiredCpus < currentCpus {
		return restartRequiredError
	}

	err := hotplugCpus(desiredCpus - currentCpus)
	if err != nil {
		logErrorMaybe(err)
		return restartRequiredError
	}
